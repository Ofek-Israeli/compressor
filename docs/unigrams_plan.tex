...
We learn one steering magnitude per selected token, plus dedicated EOS
and EOT magnitudes:
\[
\boldsymbol{\delta}=(\delta_{v_1},\dots,\delta_{v_k},\;\delta_{\text{EOS}},\;\delta_{\text{EOT}}),
\qquad v_i \in V_{\text{steer}}.
\]
$\mathrm{EOS\_ID} := \texttt{tok.eos\_token\_id}$.
$\mathrm{EOT\_ID}$ is the target tokenizer's end-of-turn token id used
as \texttt{stop\_token\_ids} during generation (e.g.\ for
Llama-3-Instruct, the \texttt{<|eot\_id|>} token).
$\delta_{\text{EOT}}\in\mathbb{R}$ is a free-sign scalar analogous to
$\delta_{\text{EOS}}$.
\textbf{Assert} $\mathrm{EOS\_ID} \neq \mathrm{EOT\_ID}$; if they are equal, terminate (otherwise EOS/EOT bias would double-apply).
...
\begin{itemize}[leftmargin=2em]
  ...
  \end{itemize}
\end{itemize}
EOS and EOT are always excluded from $V'$ (hence not in $V_{\text{steer}}$), even if \texttt{drop\_special\_tokens} is false, so their biases are not duplicated.
...
The learned parameters are $k+2$ raw scalars shared across all examples:
\[
z \in \mathbb{R}^{k+2}.
\]
The final deltas $\boldsymbol{\delta}=(\delta_{v_1},\dots,\delta_{v_k},\;\delta_{\text{EOS}},\;\delta_{\text{EOT}})$
are obtained from $z$ via a sign-constrained mapping described below.

\paragraph{Sign-constrained delta parameterization.}
$\delta_{\text{EOS}}$ and $\delta_{\text{EOT}}$ are free-sign; token deltas $\delta_v$ for $v\in V_{\text{steer}}$ remain suppression-only (non-positive).
...
For EOT (free sign, either direction):
\[
\delta_{\text{EOT}} = \delta_{\max}\tanh(z_{k+2})
\;\in\; [-\delta_{\max},\,\delta_{\max}].
\]
...
\paragraph{EOT steering.}
\label{sec:eot-steering}
The end-of-turn token ($\mathrm{EOT\_ID}$) receives its own dedicated
additive bias, applied unconditionally at every decoding step:
\[
b[\mathrm{EOT\_ID}] \;\mathrel{+}=\; \delta_{\text{EOT}}.
\]

\paragraph{Operational procedure during generation.}
The bias vector $b$ is built once from $\boldsymbol{\delta}$,
the fixed set $V_{\text{steer}}$, and the dedicated EOS and EOT terms --- it is a simple lookup table indexed by token id.
...
From the global parameters $\boldsymbol{\delta}\in\mathbb{R}^{k+2}$,
a single bias vector $b\in\mathbb{R}^{|V|}$
is \textbf{deterministically} computed via a per-token lookup into $V_{\text{steer}}$
(see Sections~\ref{sec:bias-rule}, \ref{sec:eos-steering}, and~\ref{sec:eot-steering}).
...
\[
\mathcal{L}_{\text{EOT}}(\boldsymbol{\delta};\, x)
\;:=\;
-\log P^{(b)}_{\text{LLM}}\!\big(\mathrm{EOT\_ID} \mid \pi_{T+1}\big),
\qquad
\pi_{T+1}:=(c,q,y_{1:T}).
\]
$\lambda_{\text{stop}}$ multiplies $\mathcal{L}_{\text{EOT}}$.
...
\;+\;\lambda_{\text{stop}}\,\mathcal{L}_{\text{EOT}}(\boldsymbol{\delta};\, x)
...
Saved/loaded Phase~1 parameters (for checkpoints and inference): $\{\delta_v : v \in V_{\text{steer}}\} \cup \{\delta_{\text{EOS}}, \delta_{\text{EOT}}\}$ (or equivalently the raw $z \in \mathbb{R}^{k+2}$).
\begin{itemize}[leftmargin=2em]
  ...
  \item \texttt{phase1.init.z\_eot} --- initial value for $z_{k+2}$ (EOT raw parameter), separate from steering and EOS (float).
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \texttt{loss.lambda\_stop} --- weight $\lambda_{\text{stop}}$ for $\mathcal{L}_{\text{EOT}}$ (float).
